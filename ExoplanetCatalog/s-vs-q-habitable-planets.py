...
# Load the CSV data
file_path = r'C:\Users\mgabr\Projects\CencaBridge\CENCA-Bridge-Internship\ExoplanetCatalog\ML_2025.10.13_08.13.58.csv'
...
#plotting_data = data.dropna(subset=['sy_dist', 'ml_dists'])
...
# Code for calculating peak densities areas
...
flat_indices = np.argsort(smoothed.ravel())[::-1][:N]
peak_indices = np.unravel_index(flat_indices, smoothed.shape)

# store peaks for printing
peaks = []

for i in range(N):
    yi,xi = peak_indices[0][i], peak_indices[1][i]
    # 2. Get Corresponding bin centers
    q_peak = (xedges[xi] + xedges[xi + 1]) / 2
    s_peak = (yedges[yi] + yedges[yi + 1]) / 2
    peaks.append((q_peak, s_peak))

    plt.plot(q_peak, s_peak, "r*", markersize=14, label=f'Density Peak' if i == 0 else "")
    plt.text(q_peak*1.1, s_peak, f'{i+1}', color="red", fontsize=10, fontweight='bold')

print("\nDetected density peaks (highest count bins):")

for i, (q_peak, s_peak) in enumerate(peaks, start=1):
    print(f"Peak {i}: q ≈ {q_peak:.2e}, s ≈ {s_peak:.3f}")
...
plt.xlabel('Mass Ratio, q')
plt.ylabel('s')
plt.title('Density Contour via 2D Histogram (With Density Peaks)')
